			   ...
			   struct port_options *ops)
{
	...
	if (addr < 0x03 || addr > 0x77) {
		fprintf(stderr, "I2C 地址超出范围 [0x03-0x77]\n");
	...
	h = calloc(sizeof(*h), 1);
	if (h == NULL) {
		fprintf(stderr, "到达内存末尾\n");
		return PORT_ERR_UNKNOWN;
	}
	fd = open(ops->device, O_RDWR);
	if (fd < 0) {
		fprintf(stderr, "不能打开特殊文件 \"%s\"\n",
			ops->device);
		free(h);
		return PORT_ERR_UNKNOWN;
	}
	...
	if (ret < 0) {
		fprintf(stderr, "I2C ioctl(funcs) 错误 %d\n", errno);
		close(fd);
		free(h);
		return PORT_ERR_UNKNOWN;
	}
	if ((funcs & I2C_FUNC_I2C) == 0) {
		fprintf(stderr, "错误: 控制器不是I2C, 只是 SMBUS.\n");
		close(fd);
		free(h);
		return PORT_ERR_UNKNOWN;
	}
	...
	if (ret < 0) {
		fprintf(stderr, "I2C ioctl(slave) 错误 %d\n", errno);
		close(fd);
		free(h);
		return PORT_ERR_UNKNOWN;
	}
...
static const char *i2c_get_cfg_str(struct port_interface *port)
{
	...
	if (h == NULL)
		return "无效";
	snprintf(str, sizeof(str), "地址 0x%2x", h->addr);
	return str;
}
