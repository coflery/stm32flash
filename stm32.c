...
{
	fprintf(stderr, "注意 !!!\n");
	fprintf(stderr, "\t错误  可能是由于你 I2C 控制器不能\n");
	fprintf(stderr, "\t容忍 \"bootloader 要求的时钟抖动.\"\n");
	fprintf(stderr, "\t请检查源代码中的 I2C.txt .\n");
}
...
static stm32_err_t stm32_get_ack_timeout(const stm32_t *stm, time_t timeout)
{
	...
	do {
		...
		if (byte != STM32_BUSY) {
			fprintf(stderr, "没收到 ACK, 收到 0x%02x \n",
				byte);
			return STM32_ERR_UNKNOWN;
		}
	} while (1);
					      ...
					      time_t timeout)
{
	...
	if (s_err == STM32_ERR_NACK)
		fprintf(stderr, "命令 0x%02x 收到 NACK\n", cmd);
	else
		fprintf(stderr, "命令 0x%02x 收到非预期的返回值 \n", cmd);
	return STM32_ERR_UNKNOWN;
}
...
stm32_t *stm32_init(struct port_interface *port, const char init)
{
	...
	if (stm->cmd->get == STM32_CMD_ERR
		...
		fprintf(stderr, "错误: bootloader 没有正确响应 GET 命令\n");
		return NULL;
	}
	...
	if (len < 2) {
		stm32_close(stm);
		fprintf(stderr, "PID 命令 只发送了 %d 字节 到, 未知 / 不支持设备\n", len);
		return NULL;
	}
	stm->pid = (buf[1] << 8) | buf[2];
	if (len > 2) {
		fprintf(stderr, " bootloader 响应了PID 命令 %d 个多余字节 in :", len);
		for (i = 2; i <= len ; i++)
	...
	if (!stm->dev->id) {
		fprintf(stderr, "未知 / 不支持设备 (设备 ID: 0x%03x)\n", stm->pid);
		stm32_close(stm);
		return NULL;
	}
			      ...
			      uint8_t data[], unsigned int len)
{
	...
	if (len > 256) {
		fprintf(stderr, "错误: READ 长度限制 256 字节\n");
		return STM32_ERR_UNKNOWN;
	}

	if (stm->cmd->rm == STM32_CMD_ERR) {
		fprintf(stderr, "错误: READ 命令没有被bootloader执行.\n");
			       ...
			       const uint8_t data[], unsigned int len)
{
	...
	if (address & 0x3) {
		fprintf(stderr, "错误: WRITE 地址必须是 4 字节对齐\n");
		return STM32_ERR_UNKNOWN;
	}

	if (stm->cmd->wm == STM32_CMD_ERR) {
		fprintf(stderr, "错误: WRITE 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
...
stm32_err_t stm32_wunprot_memory(const stm32_t *stm)
{
	...
	if (stm->cmd->uw == STM32_CMD_ERR) {
		fprintf(stderr, "错误: WRITE UNPROTECT c命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
	...
	if (s_err == STM32_ERR_NACK) {
		fprintf(stderr, "错误: WRITE UNPROTECT 失败\n");
		return STM32_ERR_UNKNOWN;
	}
	if (s_err != STM32_ERR_OK) {
		if (port->flags & PORT_STRETCH_W
...
stm32_err_t stm32_wprot_memory(const stm32_t *stm)
{
	...
	if (stm->cmd->wp == STM32_CMD_ERR) {
		fprintf(stderr, "错误: WRITE PROTECT 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
	...
	if (s_err == STM32_ERR_NACK) {
		fprintf(stderr, "错误: WRITE PROTECT失败\n");
		return STM32_ERR_UNKNOWN;
	}
	if (s_err != STM32_ERR_OK) {
		if (port->flags & PORT_STRETCH_W
...
stm32_err_t stm32_runprot_memory(const stm32_t *stm)
{
	...
	if (stm->cmd->ur == STM32_CMD_ERR) {
		fprintf(stderr, "错误: READOUT UNPROTECT 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
	...
	if (s_err == STM32_ERR_NACK) {
		fprintf(stderr, "错误: READOUT UNPROTECT 失败\n");
		return STM32_ERR_UNKNOWN;
	}
	if (s_err != STM32_ERR_OK) {
		if (port->flags & PORT_STRETCH_W
...
stm32_err_t stm32_readprot_memory(const stm32_t *stm)
{
	...
	if (stm->cmd->rp == STM32_CMD_ERR) {
		fprintf(stderr, "错误: READOUT PROTECT 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
	...
	if (s_err == STM32_ERR_NACK) {
		fprintf(stderr, "错误: READOUT PROTECT 失败\n");
		return STM32_ERR_UNKNOWN;
	}
	if (s_err != STM32_ERR_OK) {
		if (port->flags & PORT_STRETCH_W
...
static stm32_err_t stm32_mass_erase(const stm32_t *stm)
{
	...
	if (stm32_send_command(stm, stm->cmd->er) != STM32_ERR_OK) {
		fprintf(stderr, "不能初始化目标设备到全局擦除模式!\n");
		return STM32_ERR_UNKNOWN;
	}
	...
	if (port->write(port, buf, 3) != PORT_ERR_OK) {
		fprintf(stderr, "全局擦除失败.\n");
		return STM32_ERR_UNKNOWN;
	}
	s_err = stm32_get_ack_timeout(stm, STM32_MASSERASE_TIMEOUT);
	if (s_err != STM32_ERR_OK) {
		fprintf(stderr, "全局擦除失败. 请尝试指定要擦除的页数.\n");
	if (port->flags & PORT_STRETCH_W
	    && stm->cmd->er != STM32_CMD_EE_NS)
		stm32_warn_stretching("全局擦除");
    return STM32_ERR_UNKNOWN;
	}
	return STM32_ERR_OK;
}
...
static stm32_err_t stm32_pages_erase(const stm32_t *stm, uint32_t spage, uint32_t pages)
{
	...
	if (stm32_send_command(stm, stm->cmd->er) != STM32_ERR_OK) {
		fprintf(stderr, "不能初始化目标设备到全片擦除模式!\n");
		return STM32_ERR_UNKNOWN;
	}
	...
	free(buf);
	if (p_err != PORT_ERR_OK) {
		fprintf(stderr, "页到页擦除失败.\n");
		return STM32_ERR_UNKNOWN;
	}

	s_err = stm32_get_ack_timeout(stm, pages * STM32_PAGEERASE_TIMEOUT);
	if (s_err != STM32_ERR_OK) {
		fprintf(stderr, "页到页擦除失败. 请检查你的目标设备支持的最大页数.\n");
		if (port->flags & PORT_STRETCH_W
		    && stm->cmd->er != STM32_CMD_EE_NS)
			stm32_warn_stretching("擦除");
		return STM32_ERR_UNKNOWN;
...
stm32_err_t stm32_erase_memory(const stm32_t *stm, uint32_t spage, uint32_t pages)
{
	...
	if (stm->cmd->er == STM32_CMD_ERR) {
		fprintf(stderr, "错误: ERASE 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
				      ...
				      const uint8_t *code, uint32_t code_size)
{
	...
	if (target_address & 0x3) {
		fprintf(stderr, "错误: 代码段 地址必须是 4 字节对齐\n");
		return STM32_ERR_UNKNOWN;
	}
...
stm32_err_t stm32_go(const stm32_t *stm, uint32_t address)
{
	...
	if (stm->cmd->go == STM32_CMD_ERR) {
		fprintf(stderr, "错误: GO 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
			     ...
			     uint32_t length, uint32_t *crc)
{
	...
	if (address & 0x3 || length & 0x3) {
		fprintf(stderr, "首尾地址必须是 4 字节对齐\n");
		return STM32_ERR_UNKNOWN;
	}

	if (stm->cmd->crc == STM32_CMD_ERR) {
		fprintf(stderr, "错误: CRC 命令没有被bootloader执行.\n");
		return STM32_ERR_NO_CMD;
	}
...
uint32_t stm32_sw_crc(uint32_t crc, uint8_t *buf, unsigned int len)
{
	...
	if (len & 0x3) {
		fprintf(stderr, "缓冲区长度必须是是 4 (字节)的倍数\n");
		return 0;
	}
